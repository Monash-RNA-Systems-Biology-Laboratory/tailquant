...
output:
    html_document:
        code_folding: hide
        self_contained: true
title: "`r basename(params$in_dir)`"
---

The end of the genomic sequence in read 1 is considered identified if we see at least 12 total bases of poly(A) plus subsequent UMI and barcode.
...
```{r results="asis"}
pq <- arrow::open_dataset(params$in_dir)
...
read_1_t_lengths <- pq |>
    dplyr::filter(!is.na(sample)) |>
    dplyr::count(read_1_poly_t_length) |>
    dplyr::collect()
...
read_1_t_13 <- sum(read_1_t_lengths$n[read_1_t_lengths$read_1_poly_t_length >= 13]) / sum(read_1_t_lengths$n)
read_1_t_20 <- sum(read_1_t_lengths$n[read_1_t_lengths$read_1_poly_t_length >= 20]) / sum(read_1_t_lengths$n)
read_1_t_50 <- sum(read_1_t_lengths$n[read_1_t_lengths$read_1_poly_t_length >= 50]) / sum(read_1_t_lengths$n)
...
cat(paste0(
    ...
    scales::percent(a_long, accuracy=0.1), " poly(A) in read 1 of 13 bases or longer (among reads with identified sample and end of genomic sequence).\n\n",
    scales::percent(t_long, accuracy=0.1), " poly(T) in read 2 of 13 bases or longer (among reads with identified sample).\n\n",
    "<br>\n\n",
    scales::percent(read_1_t_13, accuracy=0.1), " poly(T) in read 1 of 13 bases or longer (among reads with identified sample).\n\n",
    scales::percent(read_1_t_20, accuracy=0.1), " poly(T) in read 1 of 20 bases or longer (among reads with identified sample).\n\n",
    scales::percent(read_1_t_50, accuracy=0.1), " poly(T) in read 1 of 50 bases or longer (among reads with identified sample).\n\n",
    "\n\n</div>\n\n"))
